...
/*
        ...
        if scrollDirection.horizontal == SZScrollDirection.FromMinToMax {
            // find cells to remove
            if borderIndexes.column.minIndex < newBorderIndexes.column.minIndex {
                for columnIndex in borderIndexes.column.minIndex ..< newBorderIndexes.column.minIndex {
                    for rowIndex in borderIndexes.row.minIndex ... borderIndexes.row.maxIndex {
            ...
            // find cells to place
            if borderIndexes.column.maxIndex <= newBorderIndexes.column.maxIndex {
                for columnIndex in borderIndexes.column.maxIndex ... newBorderIndexes.column.maxIndex {
                    for rowIndex in newBorderIndexes.row.minIndex ... newBorderIndexes.row.maxIndex {
        ...
        else if scrollDirection.horizontal == SZScrollDirection.FromMaxToMin {
            // find cells to place
            if borderIndexes.column.minIndex <= newBorderIndexes.column.minIndex {
                for columnIndex in borderIndexes.column.minIndex ... newBorderIndexes.column.minIndex {
                    for rowIndex in newBorderIndexes.row.minIndex ... newBorderIndexes.row.maxIndex {
        ...
        if scrollDirection.vertical == SZScrollDirection.FromMinToMax {
            // find cells to remove
            if borderIndexes.row.minIndex < newBorderIndexes.row.minIndex {
                for rowIndex in borderIndexes.row.minIndex ..< newBorderIndexes.row.minIndex {
                    for columnIndex in borderIndexes.column.minIndex ... borderIndexes.column.maxIndex {
                        indexesToRemove.append(SZIndexPath(rowSectionIndex: 0,
                            columnSectionIndex: 0,
                            rowIndex: rowIndex,
            ...
            // find cells to place
            if borderIndexes.row.maxIndex <= newBorderIndexes.row.maxIndex {
                for rowIndex in borderIndexes.row.maxIndex ... newBorderIndexes.row.maxIndex {
                    for columnIndex in newBorderIndexes.column.minIndex ... newBorderIndexes.column.maxIndex {
                        indexesToPlace.append(SZIndexPath(rowSectionIndex: 0,
                            columnSectionIndex: 0,
                            rowIndex: rowIndex,
                            columnIndex: columnIndex))
        ...
        else if scrollDirection.vertical == SZScrollDirection.FromMaxToMin {
            // find cells to place
            if borderIndexes.row.minIndex <= newBorderIndexes.row.minIndex {
                for rowIndex in borderIndexes.row.minIndex ... newBorderIndexes.row.minIndex {
                    for columnIndex in newBorderIndexes.column.minIndex ... newBorderIndexes.column.maxIndex {
